...
public class AuthController {
	...
	public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
		...
		if (strRoles == null) {
			...
			strRoles.forEach(role -> {
				switch (role) {
				case "admin":
					Role adminRole = roleRepository.findByName(ERole.ROLE_COURIER)
							.orElseThrow(() -> new RuntimeException("Error: Role is not found."));
					roles.add(adminRole);
	...
	public ResponseEntity<?> register(@Valid @RequestBody SignupRequest userToUpdate) {
		...
		// Create new user's account
		User user = new User(userToUpdate.getUsername(),
		...
		boolean isClient = user.getRoles().stream().collect(Collectors.toList()).get(0).getId() == 1;
		System.out.println("(saved.getId()" + saved.getId());
		if (!isClient) {
			Optional<CompanyOffice> userOffice = companyOfficeRepository.findByOfficeEmployeeByUser(saved.getId());
			//ako smeni rabotnoto mqsto
			if (userOffice.isPresent()) {
				System.out.println("!userOffice.get().getId().equals(userToUpdate.getOfficeId()) = " +!userOffice.get().getId().equals(userToUpdate.getOfficeId()));
